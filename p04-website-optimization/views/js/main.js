...
function generator(adj, noun) {
  ...
  // Reducing number of variables, return directly:
...
var pizzaElementGenerator = function(i) {
  ...
  pizzaImage.src = "images/pizza-tiny.png";
...
var resizePizzas = function(size) {
  ...
  // Replaced querySelector with getElementById to narrow the DOM manipulation.
  // So the browser can find the associated element quicker and render it on
  // the screen.
  ...
  function changePizzaSizes(size) {
    // getElementsByClassName replaces querySelectorAll for better performance
...
function updatePositions() {
  ...
  /* Setting a variable topPos to the value of document.body.scrollTop / 1250
     outside of the for loop is more efficient, because it's not needed to
     recalculate this every time.
  */
  var topPos = document.body.scrollTop / 1250;
  /* Creating the var phase outside the loop to optimize the for loop.
     Invoking the variable inside (= when needed).
     The var phase is stored as an array. Thanks to my reviewer for this one!
  */
  var phase = [];
  for (var i = 0; i < items.length; i++) {
    phase = Math.sin(topPos + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Set number of rows using the height property of the screen. Multiply
  // rows * cols to calculate the number of pizzas needed for any given window.
  var rows = window.screen.height / s;
  var totalPizzas = rows * cols;
  // Creating the var elem outside the loop so it doesn't have to be created in
  // each iteration.
  var elem = undefined;
  // Replaced querySelector with getElementById to narrow the DOM manipulation.  
